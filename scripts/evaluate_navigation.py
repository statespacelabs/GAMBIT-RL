...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--source-kind", choices=("procedural", "authored"), default="procedural")
    ...
    parser.add_argument("--mode", choices=("deterministic", "stochastic", "combat_baseline"), required=True)
...
def split_dual_observations(observations: list[np.ndarray]) -> tuple[np.ndarray, np.ndarray]:
    by_width = {int(value.shape[1]): np.asarray(value, dtype=np.float32) for value in observations if value.ndim == 2}
    if set(by_width) != {LOCAL45_DIM, ACTOR_DIM}:
        raise RuntimeError(f"dual sensor widths must be {{45,231}}, got {sorted(by_width)}")
    local45, actor = by_width[LOCAL45_DIM], by_width[ACTOR_DIM]
    if len(local45) != len(actor) or not np.isfinite(local45).all() or not np.isfinite(actor).all():
        raise RuntimeError("dual sensor transport is non-finite or row-misaligned")
    return local45, actor


def configure(args: argparse.Namespace, output: Path, seeds: list[int]) -> dict[str, Path]:
    paths = {
    ...
    environment = {
            ...
            "PHASE5_AUTONOMOUS_TARGET_SESSIONS_PER_AREA": str(args.target_sessions_per_area),
            "PHASE5_AUTONOMOUS_CHALLENGER_TIMEOUT_SECONDS": str(args.challenger_timeout_seconds),
    ...
    if args.source_kind == "procedural":
        ...
        for key in (
            ...
            "PHASE4_MAP_SEED", "PHASE4_WRITE_MAP_TELEMETRY", "PHASE4_MAP_TELEMETRY_PATH",
        ):
            os.environ.pop(key, None)
    else:
        hidden_start = args.preset in {
            "hunt_probe", "ppo_h1", "ppo_h2", "ppo_h4",
            "peek_p0", "peek_p1", "peek_p2", "peek_p3", "peek_p4",
        }
        authored_profiles = {
        ...
        environment.update({
            ...
            "PHASE4_7_ENCOUNTER_MIN_X": str(profile[0]) if hidden_start and profile[0] is not None else "",
            "PHASE4_7_ENCOUNTER_MAX_X": str(profile[1]) if hidden_start and profile[1] is not None else "",
            "PHASE4_7_ENCOUNTER_MIN_Z": str(profile[2]) if hidden_start and profile[2] is not None else "",
            "PHASE4_7_ENCOUNTER_MAX_Z": str(profile[3]) if hidden_start and profile[3] is not None else "",
        ...
        for key in (
            "PHASE5_LAYOUT_MANIFEST", "PHASE5_LAYOUT_MANIFEST_SHA256",
            "PHASE5_LAYOUT_SPLIT", "PHASE5_LAYOUT_SEEDS", "PHASE5_LAYOUT_AUDIT_PATH",
        ):
            os.environ.pop(key, None)
...
def validate_layouts(path: Path, split: str, seeds: list[int], preset: str) -> None:
    ...
    for seed in seeds:
        ...
        if preset in {"duel_probe", "ppo_h0", "reacquire_search"}:
            expected_los = True
        elif preset in {"hunt_probe", "ppo_h1", "ppo_h2", "ppo_h4", *tuple(f"peek_p{i}" for i in range(5))}:
            expected_los = False
        if expected_los is not None and bool(layout["requested_initial_los"]) != expected_los:
...
def main() -> int:
    ...
    if prerequisite.get("status") not in {
        "PHASE5_GENERIC_DAGGER_DATA_READY", "PHASE5_MAP_GENERAL_HUNTER_READY",
        "PHASE5_REACQUISITION_READY", "PHASE5_LOCAL_GEOMETRY_PEEKER_READY",
    ...
    if args.source_kind == "procedural":
        if not args.manifest or not seeds or len(seeds) != len(set(seeds)):
            raise RuntimeError("procedural evaluation requires a manifest and unique seeds")
    ...
    reacquisition_search = FairReacquisitionSearch(
        ...
        if reacquire_config else 4.0
    )
    memory_steps = int(reacquire_config.get("memory_steps", 0)) if reacquire_config else 0
    peeker_config = (navigator_payload.get("metadata") or {}).get("peeker_config")
    peeker = None
    if peeker_config:
    ...
    engine.set_configuration_parameters(time_scale=args.time_scale, target_frame_rate=-1, capture_frame_rate=0)
    expected_sessions = (len(seeds) if seeds else 1) * args.target_sessions_per_area
    challenge_count = PRESET_CHALLENGERS[args.preset]
    max_steps = max(2500, int(args.target_sessions_per_area * challenge_count * (args.challenger_timeout_seconds * 50 + 100) * 2))
    env = None
    ...
    try:
        env = UnityEnvironment(
            ...
            additional_args=(
                ["--phase5-rendered-smoke", "-logFile", str(paths["unity"])] if args.rendered
        ...
        for _ in range(max_steps):
            ...
            if len(decisions):
                ...
                actor_tensor = torch.as_tensor(actor, dtype=torch.float32, device=device).unsqueeze(1)
                if args.preset == "reacquire_search" and memory_steps > 0:
                    for index, agent_id in enumerate(ids):
                        memory_age[agent_id] = (
                            0 if actor[index, LOS_INDEX] > 0.5
                ...
                nav_batch_hidden = torch.cat(
                    [nav_hidden.get(agent_id, navigator.initial_hidden(1, device)) for agent_id in ids],
                    dim=1,
                )
                with torch.no_grad():
                ...
                finite_violation_count += int(not all(
                ...
                if args.preset == "reacquire_search" and reacquire_config:
                ...
                elif peeker is not None and args.preset.startswith("peek_p"):
                    applied, _, _, _ = peeker.apply(
                        actor, navigation, combat_actions, ids,
                    )
                else:
                    applied = compose_actions(actor, navigation, combat_actions)
                    if args.preset not in {"duel_probe", "ppo_h0"}:
                        applied, tactical_intervention = tactical_handoff.apply(actor, applied, ids)
                ...
                hidden_stuck_decisions += int(
                    (actor[hidden_mask, 26] > 0.60).sum()
                )
                for index, agent_id in enumerate(ids):
                    collision_stuck = (
                        hidden_mask[index]
                    ...
                    collision_runs[agent_id] = (
                        collision_runs.get(agent_id, 0) + 1
                ...
                visible_mismatch += int(np.any(applied[visible] != combat_actions[visible], axis=1).sum())
                aim_shoot_action_mismatches += int(np.any(
                    applied[visible, 2:8] != combat_actions[visible, 2:8], axis=1,
                ).sum())
                ...
                saturation["env"].append(float((np.abs(applied[:, :4]) >= 0.999).mean()))
            ...
            completed = sum(min(item["session"], args.target_sessions_per_area) for item in lifecycle.values())
            if len(read_jsonl(paths["trace"])) >= expected_sessions and completed >= expected_sessions:
                break
            env.step()
        else:
    ...
    contact = sum(float(row.get("time_to_first_contact_seconds", -1)) >= 0 for row in rows)
    duration = sum(max(float(row.get("session_duration_seconds", 0)), 0) for row in rows)
    stuck_seconds = sum(max(float(row.get("stuck_seconds", 0)), 0) for row in rows)
    collision_loops = sum(
        float(row.get("side_collision_count", 0)) >= 25
        and float(row.get("stuck_seconds", 0)) / max(float(row.get("session_duration_seconds", 0)), 1e-6) >= 0.03
        for row in rows
    )
    ...
    peek_metrics = peeker.summary() if peeker is not None and args.preset.startswith("peek_p") else None
    if peek_metrics is not None:
        actual_hit_events = int(sum(float(row.get("damage_inflicted", 0)) for row in rows) // 20)
        damage_positive = min(int(peek_metrics["completed_peeks"]), actual_hit_events)
        peek_metrics["damage_positive_peeks"] = damage_positive
        peek_metrics["damage_positive_peek_rate"] = (
            damage_positive / max(1, int(peek_metrics["completed_peeks"]))
        )
    result = {
        "schema_version": "phase5_navigator_live_eval_v001",
        "status": "PASS" if not failure and len(rows) == expected_sessions
        ...
        and (args.preset != "duel_probe" or visible_mismatch == 0)
        ...
        "challenger_win_rate": sum(int(row.get("kills", 0)) for row in rows) / max(
            sum(int(row.get("challengers_expected", 1)) for row in rows), 1
        ),
        ...
        "mu_saturation_rate": float(np.mean(saturation["mu"])) if saturation["mu"] else 1.0,
        "sampled_saturation_rate": float(np.mean(saturation["sampled"])) if saturation["sampled"] else 1.0,
        "environment_applied_saturation_rate": float(np.mean(saturation["env"])) if saturation["env"] else 1.0,
        ...
        "scenario_coverage": sorted({
            challenger
            for row in rows for challenger in row.get("challenger_ids", [])
        }),
        ...
        "search_event_counts": {
            name: sum(
                sum(event.get("event_name") == name for event in row.get("search_events", []))
                for row in rows
            )
            for name in (
                "LOS_LOST", "LAST_SEEN_REACHED", "LOCAL_SEARCH_STARTED",
                "REACQUIRED", "SEARCH_GOAL_STALE", "UNSTUCK_TRIGGERED",
            )
        },
    ...
    (output / "evaluation_result.json").write_text(json.dumps(result, indent=2, sort_keys=True) + "\n")
